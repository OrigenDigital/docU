...
Sub LoadCred As ResumableSub
	...
	Else
		If mResult.Get("ReqOK")=False Then
			Dim msa As Object=xui.MsgboxAsync("No hay datos config. email." & CRLF & CRLF & "Avisa al administrador de la aplicación.","Aviso")
			Wait For (msa) Msgbox_Result
			Return False
		Else
...
'									c.CellStyle
				...
				Case TipoDestinatarioInforme="Comerciales"
					...
					For j=0 To m.Size-1
						...
						If lstCamposInformeDocumentosExpedidosComerciales.IndexOf(ky)>-1 Then
			...
			Select True
				Case TipoDestinatarioInforme="Clientes"
					For j=0 To m.Size-1
						...
						If lstCamposInformePedidosPendientesClientes.IndexOf(ky)>-1 Then
							Select True
								'Case sf.InString(m.GetKeyAt(j),"Cantidad")>-1
								Case ky.Contains("Cantidad")
									FilaExcel.CreateCellNumeric(nK,m.GetValueAt(j))
				...
				Case TipoDestinatarioInforme="Comerciales"
					...
					For j=0 To m.Size-1
						...
						If lstCamposInformePedidosPendientesComerciales.IndexOf(ky)>-1 Then
							Select True
								'Case sf.InString(m.GetKeyAt(j),"Cantidad")>-1
								Case ky.Contains("Cantidad")
									FilaExcel.CreateCellNumeric(nK,m.GetValueAt(j))
