...
'</code>
Public Sub Initialize(CallBack As Object,Parent As B4XView, TipoTarifa As String, CodigoTarifa As String, Articulo As String)
	mCallBack=CallBack
	mParent=Parent
...
Sub SalirForm
	...
	frm.Close
	CallSubDelayed2(mCallBack,"cAnalisisTarifasInfoVentasTipoCodigoTarifaArticulo_Done",mDatosLineaSeleccionada)
End Sub
...
Sub Inicio
	...
	Select Case mTipoTarifa
		Case 0 
			wait for(DatosVentasCliente) complete (mResult As Map)
		Case 1
			wait for(DatosVentasGRP) complete (mResult As Map)
	End Select
