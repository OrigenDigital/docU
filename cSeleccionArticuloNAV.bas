...
'	Private btnSalir As Button
	...
	Dim mDatosArticuloSeleccionado As Map
...
Public Sub Initialize(CallBack As Object,Parent As B4XView)
	...
	mDatosArticuloSeleccionado.Initialize
	mDatosArticuloSeleccionado.Put("Articulo","")
	mDatosArticuloSeleccionado.Put("DescripcionArticulo","")
	mDatosArticuloSeleccionado.Put("ArticuloBloqueado",False)
	Show
End Sub
...
Sub Show
	'If Not(frm.IsInitialized) Then
		...
		frm.RootPane.LoadLayout("scrListaSeleccionArticuloNAV")
		
		Sleep(0)
		
		frm.Icon = Main.IconoFormularios
		
		frm.Title=Main.PrefijoTitleForms & "Selección Artículo NAV" 
		...
		Dim rSub As ResumableSub=jamTableView1.ConfigurarTableView("ListaArticulosNAV.json",Null)
		...
		jamTableView1.AddMenuItemFontAwesomeToMenuInMenuBar("Seleccionar Articulo","SeleccionarArticulo",Chr(0xF0A4),jamTableView1.MenuBarLinea)
...
Sub SalirForm
	...
	frm.Close
	CallSubDelayed2(mCallBack,"cSeleccionArticuloNAV_Done",mDatosArticuloSeleccionado)
End Sub

Sub jamTableView1_AccionSalirJamTableView
	If mDatosArticuloSeleccionado.Get("Articulo")="" Then
...
Sub DatosArticulosNav As ResumableSub
...
Sub jamTableView1_MenuBarMenuItem_Action(TagMenuItem As String)
	Select TagMenuItem
		...
		Case "SeleccionarArticulo"
			...
			SeleccionarArticulo(Dcs)
	End Select
End Sub

Sub jamTableView1_CellDobleClick(DatosCeldaSeleccionada As DatosCeldaSeleccionadaJamTableView)
	SeleccionarArticulo(DatosCeldaSeleccionada)
End Sub

Sub SeleccionarArticulo(DatosCeldaSeleccionada As DatosCeldaSeleccionadaJamTableView)
	Dim mDataPK As Map=DatosCeldaSeleccionada.mDataPK
	mDatosArticuloSeleccionado.Put("Articulo",mDataPK.Get("Articulo"))
	...
	mDatosArticuloSeleccionado.Put("DescripcionArticulo",DescripcionArticulo)
	Dim ArticuloBloqueado As Int=jamTableView1.GetValorSQLCampoFila(jamTableView1.GetIndiceFilaSeleccionada,"Bloqueado")
	mDatosArticuloSeleccionado.Put("ArticuloBloqueado",ArticuloBloqueado)
	If ArticuloBloqueado=1 Then
		Dim sb As StringBuilder
		sb.Initialize
		sb.Append("El artículo seleccionado (").Append(mDataPK.Get("Articulo")).Append(" ").Append(DescripcionArticulo).Append(") está BLOQUEADO.")
		sb.Append(CRLF).Append(CRLF)
		sb.Append("¿Continuar?")
		Dim msa As Object = xui.Msgbox2Async(sb.ToString, "", "Sí", "", "No", Null)
		Wait For (msa) Msgbox_Result (rInt As Int)
		If rInt<>xui.DialogResponse_Positive Then Return
	End If
	SalirForm
