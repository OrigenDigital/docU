...
''
''		Main.SetFormMaximized(frm)
''		'Log("DirData: " & xui.DefaultFolder)
''		'frm.SetOwner(Parent)
''		frm.RootPane.LoadLayout("scrGestionVentas")
''		
''		Sleep(0)
''		
''		frm.Icon = fx.LoadImage(File.DirAssets, "shopping-cart64.png")
''		frm.Title="Gestión Ventas   TOV v" & Main.Version & " usuario: " & Main.DatosUsuario.NombreUsuarioWindows
''		
...
''
''		jamTableViewGestionVentas.AddContextMenuItemImageColumna("Articulo","Ver Articulo NAV","VerArticuloNAV",img)
...
''	JRDCQuery.DatosJRDC(Main.rdcLinkAlmZenProin,"EmbalajesReservasActivas",Null, Me)
''	Wait For DatosJRDC_Completed (mresult As Map)
''	'lblStatus.Text=""
''		
''	If mresult.Get("ConnOK")=False Then
...
''	mReg.Put("Accion", Accion)
''	mReg.Put("lstRegistros", lstRegistros)
''	JRDCQuery.DatosJRDC(Main.rdcLinkAlmZenProtec,"EmbalajesReservasActivas",Null, Me)
''	Wait For DatosJRDC_Completed (mresult As Map)
''	'lblStatus.Text=""
''		
''	If mresult.Get("ConnOK")=False Then
...
''		End If
''	Next
''	STexto="Documento: " & Doc & " Línea: " & Linea & " Cliente: " & Cliente & " " & NombreCliente
